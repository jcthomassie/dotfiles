...
* Work in feature branch worktrees by default
* PRs get squash merged, so feature branch commit messages can be brief and tactical
* Keep commit and PR messages focused, practical, and useful
* Use stacked PRs to organize very large changes
* Use `gh` to create and manage PRs
* Use Linear MCP to read and update issues. Always reference a Linear issue (e.g. `Part of REL-123`) in PR messages when relevant. Post critical design decisions/specifications on Linear. Keep GitHub PR descriptions tactical and git-log oriented.
